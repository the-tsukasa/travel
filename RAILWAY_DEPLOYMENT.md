...
### 1. 构建失败 - Permission denied (./mvnw)

**问题**：构建时出现 `./mvnw: Permission denied` 错误

**原因**：Maven Wrapper 脚本没有执行权限

**解决方案**：
- ✅ **已修复**：`railway.json` 已配置自动添加执行权限
- 如果仍有问题，确保 `mvnw` 文件有执行权限：
  ```bash
  chmod +x mvnw
  git add mvnw
  git commit -m "添加 mvnw 执行权限"
  git push
  ```
- 或者使用 `bash mvnw` 代替 `./mvnw`
