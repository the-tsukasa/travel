...
const SpotCard = ({ spot, onUpdate }: SpotCardProps) => {
  ...
  const getSpotLikeKey = (spotId: number) => `spot_like_${spotId}`
  const getSpotFavoriteKey = (spotId: number) => `spot_favorite_${spotId}`
  ...
  const handleLike = async (e: MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      // 后端返回更新后的 spot 对象
      if (response && response.data && (response.data as Spot).likes !== undefined) {
        setLikesCount((response.data as Spot).likes)
      } else {
        // 如果没有返回数据，根据操作更新计数
  ...
  const handleFavorite = async (e: MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      // 后端返回更新后的 spot 对象
      if (response && response.data && (response.data as Spot).favorites !== undefined) {
        setFavoritesCount((response.data as Spot).favorites)
      } else {
        // 如果没有返回数据，根据操作更新计数
      ...
      <div className="spot-card-image-wrapper">
        {displayImageUrl ? (
          <img 
            ...
            loading="lazy"
            onError={(e) => {
              console.error('スポット画像読み込みエラー:', (e.target as HTMLImageElement).src)
              setImageError(true)
