...
const WeatherWidget = () => {
  ...
  const [location, setLocation] = useState<string>('渋谷区')
  ...
  // 获取白天/夜晚图标 SVG
  const getDayNightIcon = (isDay: boolean) => {
    if (isDay) {
      return (
    ...
    const fetchWeather = async () => {
      ...
      try {
        ...
        if (geoResponse.status === 'fulfilled') {
          try {
            const geoData = geoResponse.value as any
        ...
        if (weatherResponse.status === 'fulfilled') {
          try {
            const weatherData = weatherResponse.value as any
