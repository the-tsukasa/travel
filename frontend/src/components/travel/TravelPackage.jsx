...
const TravelPackage = () => {
  ...
  const updateServicePrice = (serviceKey, field, value) => {
    setServices(prev => ({
      ...prev,
      [serviceKey]: {
        ...prev[serviceKey],
      ...
      if (service.selected) {
        switch(key) {
          case 'flight':
            ...
            break
          case 'accommodation':
            total += service.price * service.nights
            break
          case 'carRental':
            total += service.price * service.days
            break
          case 'airportPickup':
            total += service.price
            break
          case 'translation':
            total += service.price * service.hours
            break
          case 'tourGuide':
            total += service.price * service.days
            break
                  ...
                  .map(([key, service]) => {
                    ...
                    switch(key) {
                      case 'flight':
                        ...
                        label = `航空券 (${service.quantity}枚)`
                        break
                      case 'accommodation':
                        serviceTotal = service.price * service.nights
                        label = `宿泊 (${service.nights}泊)`
                        break
                      case 'carRental':
                        serviceTotal = service.price * service.days
                        label = `チャーターカー (${service.days}日)`
                        break
                      case 'airportPickup':
                        ...
                        break
                      case 'translation':
                        serviceTotal = service.price * service.hours
                        label = `通訳サービス (${service.hours}時間)`
                        break
                      case 'tourGuide':
                        serviceTotal = service.price * service.days
                        label = `ガイドサービス (${service.days}日)`
                        break
                    }
                    return (
...
// 服务卡片子组件
const ServiceCard = ({ id, icon, title, service, onToggle, onUpdate, fields }) => {
  return (
