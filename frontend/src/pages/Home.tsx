...
import type { Spot, NotesDTO, PageResponse } from '../types'

const Home = () => {
  const [spots, setSpots] = useState<Spot[]>([])
  const [loading, setLoading] = useState<boolean>(true)
  const [hotspots, setHotspots] = useState<Spot[]>([])
  const [featuredSpots, setFeaturedSpots] = useState<Spot[]>([])
  const [featuredNotes, setFeaturedNotes] = useState<NotesDTO[]>([])
  ...
  const handleHotspotClick = (spotId: number) => {
    // 导航到景点详情页
