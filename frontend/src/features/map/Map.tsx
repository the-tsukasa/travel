...
// 地图边界自适应组件
function MapBounds({ spots }: MapBoundsProps) {
  const map = useMap()
...
const Map = ({ spots = [], onHotspotClick, loading = false }: MapProps) => {
  // 将 spots 转换为标记数据，并映射位置到坐标
  const markers: MarkerData[] = spots
    .map(spot => {
      if (!spot.location) return null
    ...
    .filter((marker): marker is MarkerData => marker !== null) // 过滤掉无法映射的景点
            ...
            <Popup>
              <div style={{ 
                ...
                <button
                  ...
                  onMouseOver={(e) => {
                    ;(e.target as HTMLButtonElement).style.background = '#e03e00'
                  }}
                  onMouseOut={(e) => {
